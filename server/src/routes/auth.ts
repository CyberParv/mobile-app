...
router.post('/login', authLimiter, validate({ body: { email: 'required|email', password: 'required|min:8' } }), asyncHandler(async (req, res) => {
  ...
  if (!isMatch) return res.status(401).json({ success: false, error: { code: 'INVALID_CREDENTIALS', message: 'Invalid credentials' } });
...
router.post('/refresh', validate({ body: { refreshToken: 'required' } }), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!tokenRecord || tokenRecord.revokedAt) return res.status(401).json({ success: false, error: { code: 'INVALID_REFRESH_TOKEN', message: 'Invalid refresh token' } });
